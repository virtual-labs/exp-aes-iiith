...
function doEncryption() {
  ...
  if (theForm.key.value.length * 4 != keySizeInBits) {
    alert(
      "For a " +
        keySizeInBits +
        " bit key, the hex string needs to be " +
        keySizeInBits / 4 +
        " hex characters long. But your key is of size " +
        theForm.key.value.length
    );
    if (theForm.key.select) theForm.key.select();
    return;
  }
  ...
  if (theForm.plaintext.value.length * 4 != blockSizeInBits) {
    alert(
      "For a " +
        blockSizeInBits +
        " bit block, the hex plaintext string needs to be " +
        blockSizeInBits / 4 +
        " hex characters long."
    );
    if (theForm.plaintext.select) theForm.plaintext.select();
    return;
  }
...
function doDecryption() {
  ...
  if (theForm.ciphertext.value.length * 4 != blockSizeInBits) {
    alert(
      "For a " +
        blockSizeInBits +
        " bit block, the hex ciphertext string needs to be " +
        blockSizeInBits / 4 +
        " hex characters long."
    );
    if (theForm.ciphertext.select) theForm.ciphertext.select();
    return;
  }
...
function nextKey() {
  ...
  document.getElementById("keyarea").value = randomHexString(
    chosen_option.value / 4
  );
}
...
function XOR(hex1, hex2) {
  ...
  while (l1 > -1 && l2 > -1) {
    output += (
      parseInt(hex1.charAt(l1), 16) ^ parseInt(hex2.charAt(l2), 16)
    ).toString(16);
    l1--;
    l2--;
  }
...
function checkAnswer() {
  ...
  if (chosen_option == "ecb") {
    ...
    if (!ci) {
      alert("IV is required for CBC mode");
      return;
    }
    answer += ci;
    for (i = 0; i < plaintext.length; i++) {
    ...
    if (!ri) {
      alert("IV is required for OFB mode");
      return;
    }
    answer += ri;
    for (i = 0; i < plaintext.length; i++) {
  ...
  } else if (chosen_option == "ctr") {
    var ctr = trim(document.getElementById("ctr").value);
    if (!ctr) {
      alert("CTR is required for CTR mode");
      return;
    }
    answer += ctr;
    for (i = 0; i < plaintext.length; i++) {
  ...
  if (trim(userAns) == trim(answer)) {
    notificationElement.innerHTML = "CORRECT!!";
  } else {
    notificationElement.innerHTML = "Sorry, answer is wrong. Please try again.";
  }
}
